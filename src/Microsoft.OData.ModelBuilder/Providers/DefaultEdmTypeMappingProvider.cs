...
{
    public class DefaultEdmTypeMappingProvider : IEdmTypeMappingProvider
    {
        ...
        public virtual bool TryGetEdmType(Type clrType, out IEdmPrimitiveType primitiveType)
        {
            return _builtInTypesMapping.TryGetValue(clrType, out primitiveType);
        }

        public virtual bool TryGetClrType(IEdmTypeReference edmTypeReference, out Type clrType)
        {
            clrType = null;
            ...
            foreach (KeyValuePair<Type, IEdmPrimitiveType> kvp in _builtInTypesMapping)
            {
                if (edmTypeReference.Definition.IsEquivalentTo(kvp.Value)
                    && (!edmTypeReference.IsNullable || EdmLibHelpers.IsNullable(kvp.Key)))
                {
                    clrType = kvp.Key;
                    return true;
                }
