...
namespace Microsoft.OData.ModelBuilder.Tests
{
    public class ODataModelBuilderTest
    {
        ...
        public void GetEdmModel_HasContainment_WithLowLevelModelBuilder()
        {
            // Arrange
            var builder = ODataModelBuilderMocks.GetModelBuilderMock();
            var myOrder = builder.AddEntityType(typeof(MyOrder));
            myOrder.HasKey(typeof(MyOrder).GetProperty("ID"));
            myOrder.AddContainedNavigationProperty(typeof(MyOrder).GetProperty("OrderLines"), EdmMultiplicity.Many);
            myOrder.AddContainedNavigationProperty(typeof(MyOrder).GetProperty("OrderHeader"), EdmMultiplicity.One);
            myOrder.AddContainedNavigationProperty(
                typeof(MyOrder).GetProperty("OrderCancellation"),
        ...
        public void GetEdmModel_DerivedTypeHasContainment_WithModelBuilder()
        {
            ...
            myOrder.ContainsRequired(mo => mo.OrderHeader);
            myOrder.ContainsOptional(mo => mo.OrderCancellation);
            var mySpecialOrder = builder.EntityType<MySpecialOrder>().DerivesFrom<MyOrder>();
        ...
        public void GetEdmModel_DerivedTypeHasContainment_WithLowLevelModelBuilder()
        {
            ...
            myOrder.AddContainedNavigationProperty(
                typeof(MyOrder).GetProperty("OrderCancellation"),
                EdmMultiplicity.ZeroOrOne);
            var mySpecialOrder = builder.AddEntityType(typeof(MySpecialOrder));
            mySpecialOrder.DerivesFrom(myOrder);
            mySpecialOrder.AddContainedNavigationProperty(
                typeof(MySpecialOrder).GetProperty("Gift"),
                EdmMultiplicity.ZeroOrOne);
        ...
        public void Validate_Doesnt_Throw_For_Derived_Entities()
        {
            // Arrange
            ODataModelBuilder builder = new ODataModelBuilder();
            builder.EntityType<Car>().DerivesFrom<Vehicle>();
            builder.EntityType<Vehicle>().HasKey(v => v.Name);
        ...
        public void GetEdmModel_ThrowsException_CollectionNavigationPropertyDefinedWithEntityTypeWithoutKey()
        {
            ...
            // Act & Assert
            ExceptionAssert.Throws<InvalidOperationException>(() => builder.GetEdmModel(),
                "The entity type 'Microsoft.OData.ModelBuilder.Tests.MultiRole' of " +
                "navigation property 'Roles' on structural type 'Microsoft.OData.ModelBuilder.Tests.MultiUser' " +
        ...
        public void SetNonPrimitiveProperty_ThrowsException()
        {
            ...
            ExceptionAssert.Throws<InvalidOperationException>(
                ...
                String.Format(SRResources.ReferentialConstraintPropertyTypeNotValid, "Microsoft.OData.ModelBuilder.Tests.Commons.MockType"));
        }
